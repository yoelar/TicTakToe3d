...
const GameStore_1 = require("../../components/GameStore");
describe('Win Detection Integration', () => {
    afterEach(() => {
        // Clear all games
        Object.keys(GameStore_1.games).forEach(key => delete GameStore_1.games[key]);
    });
    it('should detect horizontal win on first layer', async () => {
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            .expect(200);
        // Winning move
        const winRes = await (0, supertest_1.default)(index_1.app)
            .post(`/api/game/${gameId}/move?clientId=player-1`)
    ...
    it('should reject moves after game is won', async () => {
        ...
        const moveAfterWinRes = await (0, supertest_1.default)(index_1.app)
            .post(`/api/game/${gameId}/move?clientId=player-2`)
