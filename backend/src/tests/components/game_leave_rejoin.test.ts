...
describe('GameStore leave and rejoin scenarios', () => {
    ...
    it('1️⃣ Solo → join → continue play', () => {
        const g1 = expectCreateSuccess(createGame('p1'));
        const s1 = expectMoveSuccess(makeMove(g1.gameId, 'p1', { x: 0, y: 0, z: 0 }));

        const joinRes = expectJoinSuccess(joinGame(g1.gameId, 'p2'));
        expect(joinRes.success).toBe(true);
        expect(joinRes.player).toBe('O');

        const move2 = expectMoveSuccess(makeMove(g1.gameId, 'p2', { x: 1, y: 0, z: 0 }));
    ...
    it('2️⃣ Two players → player2 leaves → player1 continues solo', () => {
        ...
        const move2 = expectMoveSuccess(makeMove(g1.gameId, 'p1', { x: 1, y: 1, z: 1 }));
        expect(move2.success).toBe(true);
    ...
    it('3️⃣ Player1 leaves → player2 continues solo', () => {
        ...
        const move = expectMoveSuccess(makeMove(g1.gameId, 'p2', { x: 1, y: 1, z: 1 }));
    ...
    it('4️⃣ One leaves, third joins → game continues', () => {
        const g1 = expectCreateSuccess(createGame('p1'));
        expectJoinSuccess(joinGame(g1.gameId, 'p2'));
        ...
        const join3 = expectJoinSuccess(joinGame(g1.gameId, 'p3'));
        expect(join3.success).toBe(true);
        expect(join3.player).toBe('X');

        const move = expectMoveSuccess(makeMove(g1.gameId, 'p3', { x: 0, y: 0, z: 0 }));
        expect(move.success).toBe(true);
        expect(move.isFinished).toBe(false);
    });
